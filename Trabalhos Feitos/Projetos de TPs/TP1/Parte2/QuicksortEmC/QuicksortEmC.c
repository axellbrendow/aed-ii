...
void quicksort(int esqInicial, int dirInicial, char *lastResult)
{
   char *stringPivo = getNome( (Instituicao *) listGet((esqInicial + dirInicial) / 2) );
   ...
   {
       stringEsq = getNome( (Instituicao *) listGet(esquerda) );
       stringDir = getNome( (Instituicao *) listGet(direita) );
   ...
   numberOfComparisons += atoi(getAddress(getFirstString(lastResultSplited)));
   numberOfMovimentations += atoi(getAddress(getNextString(getFirstString(lastResultSplited))));
   ...
   {
       quicksort(esqInicial, direita, lastResult);
   }

   if (esquerda < dirInicial)
   {
       quicksort(esquerda, dirInicial, lastResult);
   }
}
...
int main()
{
    ...
    char *ordenationResult = startQuicksort();
