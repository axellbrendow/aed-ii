...
class Lista extends ArrayClass
{
    ...
    private void updateSize()
    {
        ...
        {
            if (array[i] == null) // checa se cada um dos elementos e' nulo
            {
    ...
    public void set(Object element, int index)
    {
        if (element == null)
        {
            //AxellIO.println("[Lista]: Parametro element nulo. - funcao set(Object, int)");
        }
        
        else if (!isMoving() && AxellIO.limitOverflow(index, size()) != 0)
        {
            AxellIO.println("[Lista]: Parametro index invalido. (index = " + index + ") - funcao set(Object, int)");
        }
...
PUC Minas - Ciencia da Computacao     Nome: Radixsort
...
public class Radixsort
{
    ...
    /**
     * Procura o maior codigo de uma lista de instituicoes
     * 
     * @param start indice de inicio da busca
     * @param institutionsList lista de instituicoes a ser percorrida
     * 
     * @return o numero de comparacoes realizadas entre elementos da lista
     * concatenado com um '\t' concatenado com o numero de movimentacoes
     * realizadas entre elementos da lista concatenado com um '\t' concatenado
     * com o indice do maior elemento a partir de start.
     */
    
    public static String getIndexOfGreatestElementStartingFrom(int start, Lista institutionsList)
    {
        int indexOfGreatestElement = start;
        int greatestElement = ((Instituicao) institutionsList.get(indexOfGreatestElement)).getCodigo();
        int currentElement;
        ...
        for (int i = start + 1; i < listSize; i++)
        {
            ...
            currentElement = ((Instituicao) institutionsList.get(i)).getCodigo();
        ...
        return numberOfComparisons + "\t" + numberOfMovimentations + "\t" + indexOfGreatestElement;
    }
    
    /**
     * Ordena uma lista de instituicoes de acordo com o algorismo da potencia
     * de 10 informada tomando como base os algorismos dos codigos das instituicoes
     * 
     * @param institutionsList lista de instituicoes a ser ordenada
     * @param tenPotence potencia de 10 do algarismo desejado. Por exemplo,
     * se tenPotence = 100, o algoritmo ira' ordenar as instituicoes de acordo
     * com o algarismo da centena dos codigos.
     * 
     * @return numero de comparacoes realizadas entre elementos da lista
     * concatenado com um '\t' concatenado com o numero de movimentacoes entre
     * elementos da lista
     */
    
    public static String countingSort(Lista institutionsList, int tenPotence)
    {
        int inputSize = institutionsList.size();
        int numberOfComparisons = 0;
        int numberOfMovimentations = 0;
        Instituicao currentInstitution;
        Lista ordenated = new Lista(institutionsList.copyList());
        
        int[] countingArray = new int[10]; // 10 = numero de algorismos na base 10
        
        for (int i = 0; i < 10; i++) // zera todas as posicoes do array de contagem
        {
            numberOfMovimentations++;
            countingArray[i] = 0;
        }
        
        // conta quantas vezes cada algarismo de cada codigo de cada instituicao aparece
        for (int i = 0; i < inputSize; i++)
        {
            currentInstitution = (Instituicao) institutionsList.get(i);
            numberOfMovimentations++;
            countingArray[ (currentInstitution.getCodigo() / tenPotence) % 10 ]++;
        }
        ...
        {
            currentInstitution = (Instituicao) institutionsList.get(i);
            numberOfMovimentations += 2;
            ordenated.set(currentInstitution, --countingArray[(currentInstitution.getCodigo() / tenPotence) % 10]);
        }
        
        System.arraycopy(ordenated.getArray(), 0, institutionsList.getArray(), 0, inputSize);
        
        return numberOfComparisons + "\t" + numberOfMovimentations;
    }
    
    public static String radixsort(Lista institutionsList, int left, int right)
    {
        int numberOfComparisons = 0;
        int numberOfMovimentations = 0;
        int greatestElement;
        ...
        methodsResult = getIndexOfGreatestElementStartingFrom(left, institutionsList).split("\t");
        numberOfComparisons += AxellIO.getint(methodsResult[0]);
        numberOfMovimentations += AxellIO.getint(methodsResult[1]);
        greatestElement = ( (Instituicao) institutionsList.get( AxellIO.getint(methodsResult[2]) ) ).getCodigo();
        
        for (int i = 1; i <= greatestElement; i *= 10)
        {
            methodsResult = countingSort(institutionsList, i).split("\t");
            numberOfComparisons += AxellIO.getint(methodsResult[0]);
            numberOfMovimentations += AxellIO.getint(methodsResult[1]);
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        Lista listaDeInstituicoes = new Lista(130);
        ...
        ordenationResult = radixsort(listaDeInstituicoes, 0, listaDeInstituicoes.size() - 1).split("\t");
        ...
        AxellIO.setPrintStream("matrícula_radixsort.txt");
