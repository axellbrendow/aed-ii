...
class FilaCircular extends ArrayClass
{
    private int first;
    private int last;
    ...
    private int getPositionInQueue(int start, int offset)
    {
    ...
    private void setFirst(int first)
    {
        ...
        {
            //AxellIO.println("[FilaCircular]: Parametro first invalido. (first = " + first + ") - funcao setFirst(int)");
        }
    ...
    /**
     * Move o indicador do primeiro elemento uma certa quantidade de posicoes
     * 
     * @param offset quantidade de posicoes a move-lo
     */
    
    private void moveFirst(int offset)
    {
        setFirst( getPositionInQueue(getFirst(), offset) );
    }
    ...
    private void setLast(int last)
    {
        ...
        {
            //AxellIO.println("[FilaCircular]: Parametro last invalido. (last = " + last + ") - funcao setLast(int)");
        }
    ...
    /**
     * Move o indicador do ultimo elemento uma certa quantidade de posicoes
     * 
     * @param offset quantidade de posicoes a move-lo
     */
    
    private void moveLast(int offset)
    {
        setLast( getPositionInQueue(getLast(), offset) );
    }
    ...
    private void setNumberOfElements(int numberOfElements)
    {
        ...
        {
            //AxellIO.println("[FilaCircular]: Parametro numberOfElements invalido. (numberOfElements = " + numberOfElements + ") - funcao setNumberOfElements(int)");
        }
    ...
    /**
     * Aumenta a quantidade de elementos da fila em uma unidade
     */
    ...
    /**
     * Diminui a quantidade de elementos da fila em uma unidade
     */
    ...
    {
        return getNumberOfElements() == length() - 1;
    }
    ...
    public void add(Object element)
    {
        if (element == null)
        {
            //AxellIO.println("[FilaCircular]: Elemento nulo. - funcao add(Object)");
        }
    ...
    public void printQueue(String delimitingString)
    {
        ...
        for (int i = 0; i < numberOfElements; i++)
        {
            AxellIO.print( get( getPositionInQueue(first, i) ).toString() + delimitingString );
        }
    }
    
    public void printQueue()
    {
        printQueue(AxellIO.LINE_SEPARATOR);
    }
    
    /**
     * Percorre os elementos da fila a partir do primeiro ate' o ultimo
     * guardando-os em sequencia num arranjo.
     * 
     * @return arranjo com os elementos da fila circular na ordem direta.
     */
    ...
    public Object[] get()
    {
        int first = getFirst();
        int numberOfElements = getNumberOfElements();
        Object[] filaCircular = new Object[numberOfElements];
        
        for (int i = 0; i < numberOfElements; i++)
        {
            filaCircular[i] = get( getPositionInQueue(first, i) );
        }
...
public class FilaCircularComAlocacaoSequencial
{
    public static int getMeanOfDespesasDocentes(FilaCircular filaCircularDeInstituicoes)
    {
        double meanOfDespesasDocentes;
        
        if (filaCircularDeInstituicoes == null)
        {
            //AxellIO.println("[FilaCircularComAlocacaoSequencial]: Fila circular de instituicoes nula. - funcao getMeanOfDespesasDocentes(FilaCircular)");
            
            meanOfDespesasDocentes = -1;
        }
        
        else
        {
            Object[] filaCircular = filaCircularDeInstituicoes.get();
            
            if (filaCircular == null || filaCircular.length == 0)
            {
                //AxellIO.println("[FilaCircularComAlocacaoSequencial]: Nao foi possivel alocar espaco para a copia da fila circular ou ela esta' vazia. - funcao getMeanOfDespesasDocentes(FilaCircular)");
            ...
            {
                meanOfDespesasDocentes = 0;
                
                for (Object object : filaCircular)
                {
                    meanOfDespesasDocentes += Instituicao.convertToInstituicao(object).getDespesaDocente();
                }
                
                meanOfDespesasDocentes /= filaCircular.length;
                meanOfDespesasDocentes = Math.round(meanOfDespesasDocentes);
            }
        }
        
        return (int) meanOfDespesasDocentes;
    }
    ...
    {
        FilaCircular filaCircularDeInstituicoes = new FilaCircular(5);
        ...
        while (lineNumber != null && lineNumber != 0)
        {
            ...
            filaCircularDeInstituicoes.add( new Instituicao(AxellIO.readLine("")) );
            
            AxellIO.println( "" + getMeanOfDespesasDocentes(filaCircularDeInstituicoes) );
        ...
        int numberOfCommands = AxellIO.readint("");
        MyString currentCommand;
        String[] commandOperands;
        
        for (int i = 0; i < numberOfCommands; i++)
        {
            currentCommand = new MyString( AxellIO.readLine("") );
            commandOperands = currentCommand.split();
            
            if (commandOperands[0].charAt(0) == 'I')
            {
                AxellIO.saveInputStreamData(); // guarda o resto dos dados da entrada padrao
                ...
                lineNumber = AxellIO.getint(commandOperands[1]);
                
                runToLine(lineNumber + 1); // vai ate a linha sucessora da linha informada na entrada padrao
                
                // cria uma instituicao com os dados dessa linha
                instituicaoAtual = new Instituicao(AxellIO.readLine(""));
                
                filaCircularDeInstituicoes.add(instituicaoAtual);
            ...
            else if (commandOperands[0].charAt(0) == 'R')
            {
                filaCircularDeInstituicoes.remove();
            }
        }
    }
}
