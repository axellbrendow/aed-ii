...
PUC Minas - Ciencia da Computacao     Nome: Heapsort
...
public class Heapsort
{
    ...
    public static int getIndexOfGreatestChild(int father, int[] intArray, int currentHeapSize, Lista institutionsList)
    {
        int greatestChild = -1;
    ...
    public static String removeFromHeap(int index, int[] intArray, int currentHeapSize, Lista institutionsList)
    {
        ...
        // obtem o indice do maior filho
        greatestChild = getIndexOfGreatestChild(index, intArray, currentHeapSize, institutionsList);
        
        if (greatestChild != -1)
        {
            siglaOfInstitutionOnNode = Instituicao.convertToInstituicao(institutionsList.get(index)).getSigla();
            siglaOfInstitutionOnChildNode = Instituicao.convertToInstituicao(institutionsList.get(greatestChild)).getSigla();
        }
        
        int i = index;
        
        numberOfComparisons++;
        // checa se existe o filho e se ele e' maior que o pai
        while (greatestChild != -1 &&
                ( intArray[greatestChild] > intArray[i] ||
                    (intArray[greatestChild] == intArray[i] &&
                    siglaOfInstitutionOnNode != null &&
        ...
        {
            swap(i, greatestChild, intArray); // troca pai com filho
            
            numberOfMovimentations += 3; // estou considerando apenas as movimentacoes da lista
            institutionsList.swap(i, greatestChild); // troca pai com filho
            
            // o indice do pai antigo passa a ser o indice do filho antigo
            i = greatestChild;
            
            // obtem o indice do maior filho
            greatestChild = getIndexOfGreatestChild(i, intArray, currentHeapSize, institutionsList);
            numberOfComparisons++;
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        /* a instituicao da linha 7 do arquivo pub.out nao tem sigla,
        // e no enunciado da questao nao fala nada sobre o que fazer
        // quando alguma instituicao nao tiver sigla.
        listaDeInstituicoes.swap(7, 8); numberOfMovimentations += 3;*/
        
        listaDeInstituicoes.printList(1);
