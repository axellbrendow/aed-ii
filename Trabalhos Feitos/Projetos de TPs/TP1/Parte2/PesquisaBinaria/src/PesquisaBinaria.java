...
PUC Minas - Ciencia da Computacao     Nome: PesquisaBinaria

Autor: Axell Brendow Batista Moreira  Matricula: 631822

Versao:  1.0                          Data: 06/09/2018
...
{
    /**
     * Faz uma pesquisa binaria num arranjo de strings de acordo com
     * a ordem lexicografica delas. E' importante ressaltar que as
     * strings ja' devem estar na ordem lexicografica para que essa
     * funcao funcione de forma coerente.
     * 
     * @param wantedString string a ser procurada
     * @param stringArray arranjo de strings a ser percorrido
     * @param start indice inicial para procurar no arranjo. Ignora elementos
     * anteriores a esse indice.
     * @param numberOfElements quantas strings para frente deseja-se procurar
     * 
     * @return quantas comparacoes foram feitas no processo. Porem, se a chave
     * tiver sido encontrada, esse numero de comparacoes estara' com o sinal
     * trocado, ou seja, negativo. Caso a string desejada ou o arranjo sejam
     * nulos, a funcao retornara' 0.
     */
    
    public static int doBinarySearch(String wantedString, String[] stringArray, int start, int numberOfElements)
    {
        int numberOfComparisons = 0;
        
        if (stringArray == null || wantedString == null)
        {
            //AxellIO.println("[PesquisaBinaria]: Arranjo ou string nulos. - doBinarySearch(String, String[], int, int)");
        }
        ...
        else
        {
            ...
            for (int left = start, right = start + numberOfElements - 1; left <= right; numberOfComparisons++)
            {
                mid = (left + right) / 2;
                middleString = stringArray[mid];
                
                if (wantedString.equals(middleString))
                {
                    numberOfComparisons *= -1;
                    left = stringArray.length;
                }
                
                else if (wantedString.compareTo(middleString) < 0)
                {
                    numberOfComparisons++;
                    right = mid - 1;
                }
    ...
    public static int doBinarySearch(String wantedString, String[] stringArray, int start)
    {
        if (stringArray == null)
        {
            //AxellIO.println("[PesquisaBinaria]: Arranjo nulo. - doBinarySearch(String, String[], int)");
            return 0;
        }
        
        else
        {
            return doBinarySearch(wantedString, stringArray, start, stringArray.length - start);
        }
    }
    
    public static int doBinarySearch(String wantedString, String[] stringArray)
    {
        return doBinarySearch(wantedString, stringArray, 0);
    }
    
    /**
     * Recebe dois indices de dois elementos e coloca um elemento no indice do outro.
     * 
     * @param elementIndex indice de um elemento
     * @param otherElementIndex indice de outro elemento
     * @param array arranjo onde os elementos estao
     * 
     * Obs.: essa funcao nao cuida de estouro do tamanho do arranjo nem checa
     * se o arranjo e' nao nulo.
     */
    
    public static void swap(int elementIndex, int otherElementIndex, Object[] array)
    {
        Object copy = array[elementIndex];
        
        array[elementIndex] = array[otherElementIndex];
        array[otherElementIndex] = copy;
    }
     ...
     * lexicograficamente, 'a string que estiver no meio do arranjo. A segunda parte,
     * mais 'a direita, tera' strings que sucedam ou sejam iguais,
     * lexicograficamente, 'a string que estiver no meio do arranjo.
     * 
     ...
     * @param stringArray arranjo de strings a ser dividido.
     */
    
    public static void quicksort(int esqInicial, int dirInicial, String[] stringArray)
    {
        String stringPivo = stringArray[(esqInicial + dirInicial) / 2];
        int esquerda = esqInicial;
        int direita = dirInicial;
        
        while (esquerda <= direita)
        {
            while (stringArray[esquerda].compareTo(stringPivo) < 0 && esquerda < dirInicial)
            {
                esquerda++;
            }
            
            while (stringArray[direita].compareTo(stringPivo) > 0 && direita > esqInicial)
            {
                direita--;
            }
            ...
            */
            if (esquerda <= direita)
            {
                swap(esquerda++, direita--, stringArray);
            }
        }
        
        if (direita > esqInicial)
        {
            quicksort(esqInicial, direita, stringArray);
        }
        
        if (esquerda < dirInicial)
        {
            quicksort(esquerda, dirInicial, stringArray);
        }
    }
     ...
     * Ordenada um arranjo de strings de acordo com a ordem lexicografica delas.
     * 
     * @param stringArray arranjo de strings a ser ordenado.
     */
    ...
    public static void quicksort(String[] stringArray)
    {
        if (stringArray == null)
        {
            //AxellIO.println("[PesquisaBinaria]: Arranjo nulo. - quicksort(String[])");
        }
        
        else
        {
            quicksort(0, stringArray.length - 1, stringArray);
        }
    }
    ...
    public static String[] getStringArrayFromObjectArray(Object[] objectArray)
    {
        ...
        if (objectArray != null)
        {
            ...
            {
                strArray[i] = objectArray[i].toString();
            }
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        Lista listaDeSiglas = new Lista(130);
        ...
        while (lineNumber != null && lineNumber != 0)
        {
            ...
            listaDeSiglas.add( new Instituicao(AxellIO.readLine("")).getSigla() );
        ...
        // transforma a lista de siglas num arranjo de strings
        String[] arrayDeSiglas = getStringArrayFromObjectArray(listaDeSiglas.copyList());
        
        // ordena o arranjo de acordo com a ordem lexicografica das siglas
        quicksort(arrayDeSiglas);
        
        String sigla = AxellIO.readLine(""); // le, da entrada padrao, a sigla a ser procurada
        int searchComparisons;
        
        while (sigla != null && !sigla.equals("FIM"))
        {
            searchComparisons = doBinarySearch(sigla, arrayDeSiglas);
            
            numberOfComparisons += Math.abs(searchComparisons);
            
            AxellIO.println( searchComparisons < 0 ? "SIM" : "NÃO" );
            
            sigla = AxellIO.readLine(""); // le, da entrada padrao, a sigla a ser procurada
        }
        
        AxellIO.setPrintStream("matrícula_binaria.txt");
