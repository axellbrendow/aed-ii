...
PUC Minas - Ciencia da Computacao     Nome: Mergesort

Autor: Axell Brendow Batista Moreira  Matricula: 631822

Versao:  1.0                          Data: 17/09/2018
...
public class Mergesort
{
    ...
    public static String merge(Lista institutionsList, int left, int middle, int right)
    {
        int i, j, k;
        int leftSize = middle - left + 1;
        int rightSize = right - middle;
        
        Lista leftList = new Lista(leftSize);
        Lista rightList = new Lista(rightSize);
        
        int numberOfComparisons = 0;
        int numberOfMovimentations = 0;
        Instituicao leftInstitution, rightInstitution;
        Double receitaLeft, receitaRight;
        
        // copia, de left 'a middle, os dados da lista original para a auxiliar
        for (i = 0; i < leftSize; i++)
        {
            numberOfMovimentations++;
            leftList.add(institutionsList.get(left + i));
        }
        
        // copia, de (middle + 1) 'a right, os dados da lista original para a auxiliar
        for (j = 0; j < rightSize; j++)
        {
            numberOfMovimentations++;
            rightList.add(institutionsList.get(middle + 1 + j));
        }
        
        i = j = 0;
        k = left;
        
        // roda ate' que todos os elementos da lista da esquerda ou da direita
        // tenham sido percorridos
        while (i < leftSize && j < rightSize)
        {
            numberOfMovimentations += 2;
            leftInstitution = ((Instituicao) leftList.get(i));
            rightInstitution = ((Instituicao) rightList.get(j));
            receitaLeft = leftInstitution.getReceita();
            receitaRight = rightInstitution.getReceita();
            
            numberOfComparisons++;
            if (receitaLeft == null || (receitaRight != null && receitaLeft < receitaRight))
            {
                i++;
                numberOfMovimentations++;
                institutionsList.set(leftInstitution, k++);
            }
            
            else if (receitaRight == null || receitaRight < receitaLeft)
            {
                j++;
                numberOfComparisons++;
                numberOfMovimentations++;
                institutionsList.set(rightInstitution, k++);
            }
            
            else
            {
                numberOfComparisons++;
                String siglaLeft = leftInstitution.getSigla();
                String siglaRight = rightInstitution.getSigla();
                
                numberOfComparisons++;
                if ( siglaLeft == null ||
                        (siglaRight != null && siglaLeft.compareTo(siglaRight) < 0) )
                {
                    i++;
                    numberOfMovimentations++;
                    institutionsList.set(leftInstitution, k++);
        ...
        // roda ate' que todos os elementos restantes da lista da esquerda
        // tenham sido colocados na lista original
        while (i < leftSize)
        {
            numberOfMovimentations++;
            institutionsList.set(leftList.get(i++), k++);
        }
        
        // roda ate' que todos os elementos restantes da lista da direita
        // tenham sido colocados na lista original
        while (j < rightSize)
        {
            numberOfMovimentations++;
            institutionsList.set(rightList.get(j++), k++);
        }
        
        return numberOfComparisons + "\t" + numberOfMovimentations;
    }
    
    public static String mergesort(Lista institutionsList, int left, int right, String compsAndMoves)
    {
        String[] mergeResult = compsAndMoves.split("\t");
        int numberOfComparisons = AxellIO.getint(mergeResult[0]);
        int numberOfMovimentations = AxellIO.getint(mergeResult[1]);
        
        if (left < right)
        {
            int middle = (left + right) / 2;
            
            mergeResult = mergesort(institutionsList, left, middle, compsAndMoves).split("\t");
            numberOfComparisons += AxellIO.getint(mergeResult[0]);
            numberOfMovimentations += AxellIO.getint(mergeResult[1]);
            
            mergeResult = mergesort(institutionsList, middle + 1, right, compsAndMoves).split("\t");
            numberOfComparisons += AxellIO.getint(mergeResult[0]);
            numberOfMovimentations += AxellIO.getint(mergeResult[1]);
            
            mergeResult = merge(institutionsList, left, middle, right).split("\t");
            numberOfComparisons += AxellIO.getint(mergeResult[0]);
            numberOfMovimentations += AxellIO.getint(mergeResult[1]);
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        ordenationResult = mergesort(listaDeInstituicoes, 0, listaDeInstituicoes.size() - 1, "0\t0").split("\t");
        ...
        AxellIO.setPrintStream("matrícula_mergesort.txt");
