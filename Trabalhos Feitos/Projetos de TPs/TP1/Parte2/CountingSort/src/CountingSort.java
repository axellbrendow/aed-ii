...
PUC Minas - Ciencia da Computacao     Nome: CountingSort
...
public class CountingSort
{
    ...
    public static Lista countingSort(Lista institutionsList)
    {
        int greatest = 0;
        ...
        for (int i = 0; i < inputSize; i++)
        {
            ...
            numberOfComparisons++;
            if (currentInstitution.getPeriodico() > greatest)
            {
                greatest = currentInstitution.getPeriodico();
            }
        }
        
        int[] countingArray = new int[greatest + 1];
        ...
        for (int i = 0; i < inputSize; i++)
        {
            currentInstitution = (Instituicao) institutionsList.get(i);
            numberOfMovimentations++;
            countingArray[ currentInstitution.getPeriodico() ]++;
        }
        ...
        for (int i = inputSize - 1; i >= 0; i--)
        {
            ...
            ordenated.set(currentInstitution, --countingArray[currentInstitution.getPeriodico()]);
        }
        
        ordenated.addOnEnd(numberOfComparisons + "\t" + numberOfMovimentations);
        
        return ordenated;
    }
    
    /**
     * Ordena uma lista de instituicoes pelas suas siglas
     * 
    ...
    public static String insertionSort(Lista institutionsList)
    {
        ...
        int numberOfComparisons = 0;
        int numberOfMovimentations = 0;
        Instituicao institutionToInsert;
        Instituicao insertedInstitution;
        int periodicOfInstitutionToInsert;
        String siglaOfInstitutionToInsert;
        String siglaOfInsertedInstitution;
        
        for (int i = 1; i < size; i++)
        {
            numberOfMovimentations++;
            institutionToInsert = ((Instituicao) institutionsList.get(i));
            periodicOfInstitutionToInsert = institutionToInsert.getPeriodico();
            siglaOfInstitutionToInsert = institutionToInsert.getSigla();
            
            j = i - 1;
            exitTime = false;
            numberOfMovimentations++;
            while ( !exitTime && (insertedInstitution = (Instituicao) institutionsList.get(j)) != null &&
                   periodicOfInstitutionToInsert == insertedInstitution.getPeriodico())
            {
                siglaOfInsertedInstitution = insertedInstitution.getSigla();
                
                numberOfComparisons++;
                if ((siglaOfInstitutionToInsert == null && siglaOfInsertedInstitution != null) ||
                    (siglaOfInstitutionToInsert != null && siglaOfInsertedInstitution != null &&
                     siglaOfInstitutionToInsert.compareTo(siglaOfInsertedInstitution) < 0))
                {
                    numberOfMovimentations++;
                    institutionsList.set(insertedInstitution, j + 1);
                    j--;
                }
            ...
            numberOfMovimentations++;
            institutionsList.set(institutionToInsert, j + 1);
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        listaDeInstituicoes = countingSort(listaDeInstituicoes);
        ordenationResult = ( (String) listaDeInstituicoes.removeOnEnd() ).split("\t");
        numberOfComparisons = AxellIO.getint(ordenationResult[0]);
        numberOfMovimentations = AxellIO.getint(ordenationResult[1]);
        
        ordenationResult = insertionSort(listaDeInstituicoes).split("\t");
        numberOfComparisons += AxellIO.getint(ordenationResult[0]);
        numberOfMovimentations += AxellIO.getint(ordenationResult[1]);
        
        listaDeInstituicoes.printList();
        
        AxellIO.setPrintStream("matrícula_countingsort.txt");
