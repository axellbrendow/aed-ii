...
PUC Minas - Ciencia da Computacao     Nome: Shellsort
...
{
    public static String getInstitutionUf(Object institution)
    {
        return Instituicao.convertToInstituicao(institution).getUf();
    }
    
    public static String getInstitutionAbbreviation(Object institution)
    {
        return Instituicao.convertToInstituicao(institution).getSigla();
    }
    ...
    public static String shellsort(Lista institutionsList)
    {
        int elementsMargin = 1;
        int numberOfInstitutions = institutionsList.size();
        
        do
        {
            elementsMargin = elementsMargin * 3 + 1;
            
        } while (elementsMargin < numberOfInstitutions);
        
        int numberOfComparisons = 0;
        int numberOfMovimentations = 0;
        Instituicao currentInstitution, previousInstitution;
        String currentUf, previousUf;
        String currentAbbreviation, previousAbbreviation;
        boolean exitTime;
        int j;
        
        while ((elementsMargin /= 3) >= 1)
        {
            for (int color = 0; color < elementsMargin; color++)
            {
                for (int i = color + elementsMargin; i < numberOfInstitutions; i += elementsMargin)
                {
                    numberOfMovimentations++;
                    currentInstitution = (Instituicao) institutionsList.get(i);
                    currentUf = currentInstitution.getUf();
                    currentAbbreviation = currentInstitution.getSigla();
                    ...
                    {
                        numberOfMovimentations++;
                        previousInstitution = (Instituicao) institutionsList.get(j);
                        previousUf = previousInstitution.getUf();
                        
                        numberOfComparisons++;
                        if (currentUf.equals(previousUf))
                        {
                            previousAbbreviation = previousInstitution.getSigla();
                            
                            if ((currentAbbreviation != null && previousAbbreviation == null) ||
                                (currentAbbreviation != null && currentAbbreviation.compareTo(previousAbbreviation) > 0))
                            {
                                exitTime = true;
                            }
                            ...
                            {
                                numberOfMovimentations++;
                                institutionsList.set(previousInstitution, j + elementsMargin);
                            }
                        }
                        
                        else if (currentUf.compareTo(previousUf) > 0)
                        {
                            numberOfComparisons++;
                            exitTime = true;
                        }
                    ...
                    } while ( !exitTime && (j -= elementsMargin) >= 0 );
                    
                    numberOfMovimentations += 3;
                    institutionsList.set(currentInstitution, j + elementsMargin);
                }
            }
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        ordenationResult = shellsort(listaDeInstituicoes).split("\t");
        ...
        AxellIO.setPrintStream("matrícula_shellsort.txt");
