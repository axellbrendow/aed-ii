...
PUC Minas - Ciencia da Computacao     Nome: Bubblesort
...
public class Bubblesort
{
    ...
    public static String bubblesort(Lista institutionsList)
    {
        int numberOfComparisons = 0;
        int numberOfMovimentations = 0;
        int numberOfInstitutions = institutionsList.size();
        int ordenatedPartSize = 0;
        Instituicao instituicaoBolha, instituicaoAntecessora;
        int livroBolha, livroAntecessor;
        String siglaBolha, siglaAntecessor;
        
        while (ordenatedPartSize != numberOfInstitutions)
        {
            for (int bolha = numberOfInstitutions - 1; bolha > ordenatedPartSize; bolha--)
            {
                numberOfMovimentations += 2;
                instituicaoBolha = (Instituicao) institutionsList.get(bolha);
                instituicaoAntecessora = (Instituicao) institutionsList.get(bolha - 1);
                livroBolha = instituicaoBolha.getLivro();
                livroAntecessor = instituicaoAntecessora.getLivro();
                
                numberOfComparisons++;
                if (livroBolha == livroAntecessor)
                {
                    siglaBolha = instituicaoBolha.getSigla();
                    siglaAntecessor = instituicaoAntecessora.getSigla();
                    
                    numberOfComparisons++;
                    if (siglaBolha == null ||
                       (siglaAntecessor != null && siglaBolha.compareTo(siglaAntecessor) < 0))
                    {
                        numberOfMovimentations += 3;
                        institutionsList.swap(bolha, bolha - 1);
                    }
                }
                
                else if (livroBolha < livroAntecessor)
                {
                    numberOfComparisons++;
                    numberOfMovimentations += 3;
                    institutionsList.swap(bolha, bolha - 1);
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        ordenationResult = bubblesort(listaDeInstituicoes).split("\t");
        ...
        AxellIO.setPrintStream("matrícula_bolha.txt");
