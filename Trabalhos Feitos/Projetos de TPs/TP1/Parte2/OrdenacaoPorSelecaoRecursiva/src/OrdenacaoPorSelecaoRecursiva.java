...
PUC Minas - Ciencia da Computacao     Nome: OrdenacaoPorSelecaoRecursiva

Autor: Axell Brendow Batista Moreira  Matricula: 631822

Versao:  1.0                          Data: 07/09/2018

*/

public class OrdenacaoPorSelecaoRecursiva
{
    public static int[] getInstitutionsCodes(Lista institutionsList)
    {
        ...
        {
            institutionsCodes[i] = Instituicao.convertToInstituicao( institutionsList.get(i) ).getCodigo();
        }
    ...
    /**
     * Procura o menor elemento de um arranjo de inteiros.
     * 
     * @param start indice de inicio da busca
     * @param intArray arranjo de inteiros a ser percorrido
     * 
     * @return indice do menor elemento concatenado com um '\t' concatenado
     * com a quantidade de comparacoes realizadas entre os elementos do arranjo.
     */
    
    public static String getIndexOfLowestElementStartingFrom(int start, int[] intArray, int lowestElement, int indexOfLowestElement, int numberOfComparisons, String lastResult)
    {
        ...
        if (start < intArray.length)
        {
            ...
            if (intArray[start] < lowestElement)
            {
                lowestElement = intArray[start];
                indexOfLowestElement = start;
                result = indexOfLowestElement + "\t" + numberOfComparisons;
            }
            
            result = getIndexOfLowestElementStartingFrom(start + 1, intArray, lowestElement, indexOfLowestElement, numberOfComparisons, result);
        }
    ...
    /**
     * Ordena uma lista de instituicoes pelos seus codigos.
     * 
    ...
    public static String ordenateInstitutionsByCode(Lista institutionsList, int size, int i, int[] codesArray, int numberOfComparisons, int numberOfMovimentations, String lastResult)
    {
        String[] resultOfSearchForLowestElement;
        int indexOfLowestElement;
        String result = lastResult;
        
        if (i < size)
        {
            resultOfSearchForLowestElement = getIndexOfLowestElementStartingFrom(i, codesArray, 1000000, i, 0, i + "\t" + 0).split("\t");
            
            indexOfLowestElement = AxellIO.getint(resultOfSearchForLowestElement[0]);
            numberOfComparisons += AxellIO.getint(resultOfSearchForLowestElement[1]);
            
            numberOfMovimentations += 3;
            institutionsList.swap(i, indexOfLowestElement); // estou considerando apenas as movimentacoes da lista
            swap(i, indexOfLowestElement, codesArray); // estou desconsiderando essas movimentacoes
            
            result = numberOfComparisons + "\t" + numberOfMovimentations;
            
            result = ordenateInstitutionsByCode(institutionsList, size, i + 1, codesArray, numberOfComparisons, numberOfMovimentations, result);
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        ordenationResult = ordenateInstitutionsByCode(listaDeInstituicoes, listaDeInstituicoes.size(), 0, getInstitutionsCodes(listaDeInstituicoes), 0, 0, 0 + "\t" + 0).split("\t");
        ...
        AxellIO.setPrintStream("matrícula_selecaoRecursiva.txt");
