...
PUC Minas - Ciencia da Computacao     Nome: AquecimentoRecursivo
...
{
    public static int countUpperCases(MyString string)
    {
        int upperCases = 0;

        if (!string.equals(""))
        {
            if (AxellIO.isUpperCase(string.getHead())) // checa se o caractere primeiro caractere da string e' uma letra maiscula
            {
                upperCases++; // se for, aumenta uma unidade na quantidade de maiusculas
            }

            // soma 'a quantidade de maiusculas a quantidade que for encontrada depois da letra atual
            upperCases += countUpperCases( new MyString(string.getTail()) );
        }
    ...
    public static void main(String[] args)
    {
        ...
        {
            AxellIO.println("" + countUpperCases(input));
