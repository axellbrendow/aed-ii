...
PUC Minas - Ciencia da Computacao     Nome: HashTableWithReserve
...
class HashTableWithReserve
{
    ...
    public HashTableWithReserve(int hashSize, int reserveSize)
    {
        if (hashSize < 1)
        {
            //AxellIO.println("[HashTableWithReserve]: Tamanho da tabela hash < 1. - construtor HashTableWithReserve(int, int)");
        }
        
        else if (reserveSize < 1)
        {
            //AxellIO.println("[HashTableWithReserve]: Tamanho da reserva < 1. - construtor HashTableWithReserve(int, int)");
        }
        ...
        else
        {
            ...
            table = new Object[hashSize + reserveSize];
        }
    }
    ...
    public void insert(int element)
    {
        ...
        Object elementOnIndex = table[hashIndex];
        
        if (elementOnIndex == null)
        {
            table[hashIndex] = element;
        }
        
        else if (reserveCounter < reserveSize)
        {
            table[hashSize + reserveCounter++] = element;
        }
    }
    ...
    public int search(int element)
    {
        ...
        {
            numberOfComparisons++;
            for (int i = hashSize; i < table.length && elementIndex == -1; i++)
            {
                if ( (int) table[i] == element )
                {
                    elementIndex = i;
                }
...
PUC Minas - Ciencia da Computacao     Nome: TabelaHashDiretaComReserva
...
public class TabelaHashDiretaComReserva
{
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        HashTableWithReserve hashTable = new HashTableWithReserve(21, 9);
        ...
        while (lineNumber != null && lineNumber != 0)
        {
            ...
            hashTable.insert( new Instituicao(AxellIO.readLine("")).getCodigoMantenedora() );
        ...
        AxellIO.setPrintStream("631822_hashReserva.txt");
