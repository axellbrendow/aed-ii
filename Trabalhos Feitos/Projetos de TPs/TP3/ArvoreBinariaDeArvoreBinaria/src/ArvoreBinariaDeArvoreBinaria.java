...
PUC Minas - Ciencia da Computacao     Nome: BinarySearchTree
...
class BinarySearchTree
{
    ...
    public void insert(Object element)
    {
        ...
        {
            case Strings:
                setRoot( insert((String) element, getRoot()) );
                break;
                
            case Numbers:
                setRoot( insert((Double) element, getRoot()) );
                break;
...
PUC Minas - Ciencia da Computacao     Nome: ArvoreBinariaDeArvoreBinaria
...
public class ArvoreBinariaDeArvoreBinaria
{
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        BinarySearchTree binaryTree = new BinarySearchTree(BinarySearchTree.TreeType.Numbers);
        {
            binaryTree.insert(7);
            binaryTree.insert(3);
        ...
        while (lineNumber != null && lineNumber != 0)
        {
            ...
            institutionCode = new Instituicao(AxellIO.readLine("")).getCodigo();
            
            // adiciona-a 'a arvore binaria de arvore binaria de instituicoes.
            binaryTree.search(institutionCode % 15).getBinaryTree().insert(institutionCode);
        ...
        Integer code = AxellIO.readInt(""); // le, da entrada padrao, o codigo a ser procurado
        BinarySearchTree codeTree;
        Node codeNode;
        String bTPath;
        
        while (code != null)
        {
            codeTree = binaryTree.search(code % 15).getBinaryTree();
            numberOfComparisons += binaryTree.numberOfComparisons;
            codeNode = codeTree.search(code);
            numberOfComparisons += codeTree.numberOfComparisons;
            
            bTPath = binaryTree.searchPath;
            bTPath = bTPath.substring(0, bTPath.length() - 3);
            
            AxellIO.println( bTPath + codeTree.searchPath );
            
            code = AxellIO.readInt(""); // le, da entrada padrao, o codigo a ser procurado
        }
        
        AxellIO.setPrintStream("matrícula_arvoreArvore.txt");
