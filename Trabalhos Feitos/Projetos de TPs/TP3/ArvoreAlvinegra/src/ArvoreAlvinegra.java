...
PUC Minas - Ciencia da Computacao     Nome: Node

Autor: Axell Brendow Batista Moreira  Matricula: 631822

Versao:  1.0                          Data: 24/10/2018
...
class Node
{
    ...
    public Node(Object element, Node left, Node right, boolean color)
    {
...
PUC Minas - Ciencia da Computacao     Nome: AlvinegraBinarySearchTree
...
class AlvinegraBinarySearchTree
{
    Node root;
    TreeType treeType;
    String searchPath;
    ...
    public AlvinegraBinarySearchTree(TreeType treeType)
    {
    ...
    private Node rotateRightToRightThenRotateToLeft(Node node) // R - L
    {
    ...
    private Node rotateLeftToLeftThenRotateToRight(Node node) // L - R
    {
    ...
    private boolean lessThan(Node node1, Node node2)
    {
        boolean less;
        ...
        {
            case Numbers:
                less = (Double) node1.getElement() < (Double) node2.getElement();
                break;
                
            case Strings:
                less = ( (String) node1.getElement() ).compareTo( (String) node2.getElement() ) < 0;
                break;
    ...
    private void balanceFamily(Node greatGF, Node gF, Node f, Node current, char direction)
    {
        // como a cor da raiz (false) nunca muda, se o no' atual ou o seu pai forem a raiz,
        // e' impossivel que eles entrem no if
        if (current.getColor() == true && f.getColor() == true)
        {
            boolean gF_was_the_root = gF == getRoot();
            
            if (lessThan(f, gF)) // no' pai 'a esquerda do avo
            {
                if (direction == 'l') // current 'a esquerda do no' pai (rotacao simples 'a direita)
                {
                    gF = rotateToRight(gF);
                }
                
                else // current 'a direita do no' pai (rotacao dupla esquerda direita)
                {
                    gF = rotateLeftToLeftThenRotateToRight(gF);
                }
            }
            
            else // no' pai 'a direita do avo
            {
                if (direction == 'r') // current 'a direita do no' pai (rotacao simples 'a esquerda)
                {
                    gF = rotateToLeft(gF);
                }
                
                else // current 'a esquerda do no' pai (rotacao dupla direita esquerda)
                {
                    gF = rotateRightToRightThenRotateToLeft(gF);
                }
            }
            
            gF.setColor(false);
            gF.getLeft().setColor(true);
            gF.getRight().setColor(true);
            
            if (gF_was_the_root) // antigo avo era a raiz
            {
                setRoot( gF ); // atualiza a raiz com o novo avo
            }

            else
            {
                // avo 'a esquerda do bisavo
                if (lessThan(gF, greatGF))
                {
                    greatGF.setLeft( gF );
                }

                else // avo 'a direita do bisavo
                {
                    greatGF.setRight( gF );
                }
            }
    ...
    // Obs.: greatGF = great grand father (bisavo)
    private void insert(String str, Node greatGF, Node gF, Node f, Node current, char direction)
    {
        if (current == null)
        {
            current = new Node(str, null, null, true);
            ...
            balanceFamily(greatGF, gF, f, current, direction);
        }
        ...
        else
        {
            ...
            if (currentLeft != null && currentRight != null &&
                currentLeft.getColor() == true && currentRight.getColor() == true)
            {
                currentLeft.setColor(false);
                currentRight.setColor(false);
            ...
            {
                insert(str, gF, f, current, current.getLeft(), 'l');
            }
            
            else if (str.compareTo(currentStr) > 0)
            {
                insert(str, gF, f, current, current.getRight(), 'r');
            }
        }
    }
    
    // Obs.: greatGF = great grand father (bisavo)
    private void insert(Double num, Node greatGF, Node gF, Node f, Node current, char direction)
    {
        if (current == null)
        {
            current = new Node(num, null, null, true);
        ...
        else
        {
            ...
            {
                insert(num, gF, f, current, current.getLeft(), 'l');
            }
            
            else if (num.compareTo(currentNum) > 0)
            {
                insert(num, gF, f, current, current.getRight(), 'r');
            }
        }
    }
    ...
    public void insert(Object element)
    {
        ...
        if (root == null) // checa se nao existe raiz
        {
            setRoot( new Node(element, null, null, false) );
        }
        ...
        else // caso exista,
        {
            ...
            if ( !element.equals(root.getElement()) )
            {
                ...
                {
                    case Strings:
                        direction = ( ((String) element).compareTo( (String) root.getElement() ) < 0 ? 'l' : 'r' );
                        insert( (String) element, null, null, root, ( direction == 'l' ? rootLeft : rootRight ), direction );
                        break;
                        
                    case Numbers:
                        direction = ( (Double) element < (Double) root.getElement() ? 'l' : 'r' );
                        insert( (Double) element, null, null, root, ( direction == 'l' ? rootLeft : rootRight ), direction );
                        break;
                ...
                if (rootLeft != null && rootRight != null &&
                    rootLeft.getColor() == true && rootRight.getColor() == true)
                {
                    rootLeft.setColor(false);
                    rootRight.setColor(false);
                }
            }
        }
    }
    ...
    public Node search(Object element)
    {
        ...
        {
            case Strings:
                result = search((String) element, getRoot());
                break;
                
            case Numbers:
                result = search((Double) element, getRoot());
                break;
        ...
        searchPath += ( result == null ? "NÃO" : "SIM" );
        
        return result;
    }
    
    public void mostrarPre(Node node)
    {
        if (node != null)
        {
            AxellIO.print(node.getElement() + " ");
            mostrarPre(node.getLeft());
            mostrarPre(node.getRight());
        }
    }
...
PUC Minas - Ciencia da Computacao     Nome: ArvoreAlvinegra
...
public class ArvoreAlvinegra
{
    ...
    {
        AlvinegraBinarySearchTree alv = new AlvinegraBinarySearchTree(AlvinegraBinarySearchTree.TreeType.Numbers);
        alv.insert(1000.0);
        alv.mostrarPre();
        alv.insert(2000.0);
        alv.mostrarPre();
        ...
        alv.mostrarPre();
        alv.search(7000.0);
        AxellIO.println(alv.searchPath);
        /*long start = System.currentTimeMillis();
        
        int numberOfComparisons = 0;
        
        AlvinegraBinarySearchTree binaryTree = new AlvinegraBinarySearchTree(AlvinegraBinarySearchTree.TreeType.Strings);
        ...
        while (lineNumber != null && lineNumber != 0)
        {
            ...
            binaryTree.insert( new Instituicao(AxellIO.readLine("")).getSigla() );
        ...
        {
            siglaNode = binaryTree.search(sigla);
            
            numberOfComparisons += binaryTree.numberOfComparisons;
            
            AxellIO.println( binaryTree.searchPath );
            
            sigla = AxellIO.readLine(""); // le, da entrada padrao, a sigla a ser procurada
        }
        
        AxellIO.setPrintStream("631822_avinegra.txt");
