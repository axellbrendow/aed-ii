...
PUC Minas - Ciencia da Computacao     Nome: SimpleList
...
class SimpleList
{
    ...
    public Object removeOnEnd()
    {
        ...
        if (!isEmpty())
        {
            ...
            Cell last = getLast();
            Cell previous = getCellOnIndex(numberOfElements - 2);
    ...
    public Object remove(int index)
    {
        ...
        else
        {
            ...
            if (previousOfRemovedCell != null)
            {
                ...
                previousOfRemovedCell.setNext(removedCell.getNext());
    ...
    public Object removeSecondPosition()
    {
        return remove(1);
    }
    ...
    public void add(Object element, int index)
    {
        ...
        else
        {
            ...
            Cell newCell = new Cell(element, cellOnIndex);
    ...
    public int find(Object element)
    {
        ...
        if (!isEmpty())
        {
            ...
            for (Cell i = getFirst(); i != null && index == -1; i = i.getNext(), currentIndex++)
            {
...
PUC Minas - Ciencia da Computacao     Nome: HashTableWithSimpleList
...
class HashTableWithSimpleList
{
    SimpleList[] table;
    int numberOfComparisons;
    
    public HashTableWithSimpleList(int hashSize)
    {
        if (hashSize < 1)
        {
            //AxellIO.println("[HashTableWithSimpleList]: Tamanho da tabela hash < 1. - construtor HashTableWithSimpleList(int)");
        }
        
        else
        {
            table = new SimpleList[hashSize];
    ...
    public void insert(Pair pair)
    {
        ...
        table[hashIndex].add(pair.getValue());
    }
    
    public int search(int element)
    {
        int hashIndex = hash(element);
        SimpleList list = table[hashIndex];
        int elementIndex = list.find(element);
        
        numberOfComparisons = (elementIndex == -1 ? list.getNumberOfElements() : elementIndex + 1);
    ...
    public int search(Object element)
    {
        ...
        for (i = 0; elementIndex == -1 && i < table.length; i++)
        {
            list = table[i];
            elementIndex = list.find(element);
            
            numberOfComparisons += (elementIndex == -1 ? list.getNumberOfElements() : elementIndex + 1);
        }
        
        return ( elementIndex == -1 ? -1 : i - 1 );
    }
...
PUC Minas - Ciencia da Computacao     Nome: TabelaHashIndiretaComListaSimples
...
public class TabelaHashIndiretaComListaSimples
{
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        HashTableWithSimpleList hashTable = new HashTableWithSimpleList(21);
        ...
        Integer institutionMaintainerCode;
        String institutionInitials;
        ...
        while (lineNumber != null && lineNumber != 0)
        {
            ...
            institutionMaintainerCode = currentInstitution.getCodigoMantenedora();
            institutionMaintainerCode = ( institutionMaintainerCode == null ? 0 : institutionMaintainerCode );
            institutionInitials = currentInstitution.getSigla();
            institutionInitials = ( institutionInitials == null ? "" : institutionInitials );
            
            // adiciona-a 'a lista de instituicoes.
            hashTable.insert( new Pair( institutionMaintainerCode, institutionInitials ) );
        ...
        while (sigla != null && !sigla.equals("FIM"))
        {
            if (!sigla.equals(""))
            {
                hashIndex = hashTable.search(sigla);
                
                numberOfComparisons += hashTable.numberOfComparisons;

                AxellIO.println( sigla + "\t" + ( hashIndex == -1 ? "NÃO" : "(" + hashIndex + ") SIM" ) );
            }
            
            sigla = AxellIO.readLine(""); // le, da entrada padrao, a sigla a ser procurada
        }
        
        AxellIO.setPrintStream("631822_hashIndireta.txt");
