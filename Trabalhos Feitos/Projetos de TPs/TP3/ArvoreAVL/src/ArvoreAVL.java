...
class Node
{
    ...
    Node left;
    Node right;
    int nodesBelowByLeftPath;
    int nodesBelowByRightPath;
    int nodesBelow;
    ...
    public Node(Object element, Node left, Node right, int nodesBelow)
    {
    ...
    public int getNodesBelow()
    {
        return nodesBelow;
    }
    
    public void setNodesBelow(int nodesBelow)
    {
        this.nodesBelow = nodesBelow;
    }
    
    /**
     * Alem de atualizar a quantidade de nos abaixo deste, esta funcao tambem
     * atualiza a quantidade de nos abaixo indo pela esquerda e pela direita.
     */
    
    public void updateNodesBelow()
    {
        // atualiza a quantidade de nos 'a esquerda e 'a direita
        nodesBelowByLeftPath = ( left == null ? 0 : left.getNodesBelow() + 1 );
        nodesBelowByRightPath = ( right == null ? 0 : right.getNodesBelow() + 1 );
        
        // pega o maior valores entre os dois para ser a quantidade de nos abaixo deste
        setNodesBelow( Math.max( nodesBelowByLeftPath, nodesBelowByRightPath ) );
    }
    
    /**
     * Atualiza o fator de balanceamento do no'
     * 
    ...
    public int updateBalanceFactor()
    {
        ...
        return nodesBelowByRightPath - nodesBelowByLeftPath;
    }
...
PUC Minas - Ciencia da Computacao     Nome: AVLBinarySearchTree
...
class AVLBinarySearchTree
{
    ...
    public AVLBinarySearchTree(TreeType treeType)
    {
    ...
    /**
     * Se o no' recebido estiver com fator de balanceamento 2 ou -2, essa
     * funcao retorna um novo no' que deve ser colocado no lugar do no'
     * recebido para que a subarvore no lugar do no' recebido fique balanceada.
     * Caso contrario, a funcao retorna o proprio no' recebido.
     * 
     * @param node no' cuja subarvore deseja-se balancear
     * 
     * @return um novo no' que deve ser colocado no lugar do no' recebido
     * para que a subarvore no lugar do no' recebido fique balanceada.
     * Caso contrario, a funcao retorna o proprio no' recebido.
     */
    
    private Node balanceSubTree(Node node)
    {
        int nodeBalanceFactor = node.updateBalanceFactor();
        
        if (nodeBalanceFactor == 2) // subarvore do no' com galho 'a direita
        {
            // subarvore do no' 'a direita com galho 'a esquerda
            if (node.getRight().getBalanceFactor() == -1)
            {
                node = rotateRightToRightThenRotateToLeft(node);
            }
            ...
            node.getLeft().updateBalanceFactor();
            node.getRight().updateBalanceFactor();
            node.updateBalanceFactor();
        }
        
        else if (nodeBalanceFactor == -2) // subarvore do no' com galho 'a esquerda
        {
            // subarvore do no' 'a esquerda com galho 'a direita
            if (node.getLeft().getBalanceFactor() == 1)
            {
                node = rotateLeftToLeftThenRotateToRight(node);
            }
    ...
    // retorna o proprio no' atual ou um novo no' que sera' filho do no' anterior
    // ou um novo no' que sera' o substituto do no' atual por questoes de
    ...
    private Node insert(String str, Node current)
    {
        ...
        current = balanceSubTree(current);
    ...
    // ou um novo no' que sera' o substituto do no' atual por questoes de
    // balanceamento da arvore
    private Node insert(Double num, Node current)
    {
        if (current == null)
        {
            current = new Node();
            current.setElement(num);
        }
        ...
        else
        {
            ...
            {
                current.setLeft( insert(num, current.getLeft()) );
            }
            
            else if (num.compareTo(currentNum) > 0)
            {
                current.setRight( insert(num, current.getRight()) );
            }
        }
    ...
    public void insert(Object element)
    {
        ...
        {
            case Strings:
                setRoot( insert(element == null ? "" : (String) element, getRoot()) );
                break;
                
            case Numbers:
                setRoot( insert(element == null ? 0.0 : (Double) element, getRoot()) );
                break;
    ...
    public void mostrarPre(Node current)
    {
        if (current != null)
        {
            AxellIO.print(", " + current.getElement() + " (" + current.getBalanceFactor() + ")");
            
            mostrarPre(current.getLeft());
            mostrarPre(current.getRight());
        }
    }
...
PUC Minas - Ciencia da Computacao     Nome: ArvoreAVL
...
public class ArvoreAVL
{
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        AVLBinarySearchTree binaryTree = new AVLBinarySearchTree(AVLBinarySearchTree.TreeType.Strings);
        ...
        while (sigla != null && !sigla.equals("FIM"))
        {
            if (!sigla.equals(""))
            {
                ...
                AxellIO.println( sigla + " " + binaryTree.searchPath );
            }
            
            sigla = AxellIO.readLine(""); // le, da entrada padrao, a sigla a ser procurada
        }
        
        AxellIO.setPrintStream("631822_avl.txt");
