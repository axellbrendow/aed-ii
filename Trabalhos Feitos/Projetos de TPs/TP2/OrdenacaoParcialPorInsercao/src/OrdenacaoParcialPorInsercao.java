...
PUC Minas - Ciencia da Computacao     Nome: OrdenacaoParcialPorInsercao
...
public class OrdenacaoParcialPorInsercao
{
    public static int[] getInstitutionsCodes(Lista institutionsList)
    {
        ...
        {
            institutionsCodes[i] = Instituicao.convertToInstituicao( institutionsList.get(i) ).getCodigoMantenedora();
        }
    ...
    /**
     * Ordena uma lista de instituicoes pelos codigos de suas mantenedoras.
     * 
    ...
    public static String ordenateInstitutionsByCode(Lista institutionsList, int k)
    {
        ...
        {
            elementToInsert = codesArray[i]; // pega o codigo mantenedora em "i"
            objectToInsert = institutionsList.get(i); // pega a instituicao em "i"
            numberOfMovimentations++; // conta a primeira movimentacao
            // (estou considerando apenas as movimentacoes da lista)
            
            // checa se "i" ja' passou do indice da ultima posicao de interesse
            if (i > k - 1)
            {
                j = k - 1; // volta com o "j" para a ultima posicao de interesse
                
                if (codesArray[i] < codesArray[k])
                {
                    swap(i, k, codesArray);
                    institutionsList.swap(i, k);
                }
            }
            ...
            numberOfComparisons++; // conta a primeira checagem do for
            for (; elementToInsert < codesArray[j] && j >= 0; j--, numberOfComparisons++)
            {
                codesArray[j + 1] = codesArray[j];
                institutionsList.set(institutionsList.get(j), j + 1);
                numberOfMovimentations++;
            }
            
            siglaOfInstitutionToInsert = Instituicao.convertToInstituicao(objectToInsert).getSigla();
            siglaOfInsertedInstitution = Instituicao.convertToInstituicao(institutionsList.get(j)).getSigla();
            
            // checa a existencia de dois codigos mantenedoras iguais
            numberOfComparisons++;
            if (elementToInsert == codesArray[j] &&
                    siglaOfInstitutionToInsert != null &&
                    siglaOfInsertedInstitution != null &&
                    siglaOfInstitutionToInsert.compareTo(siglaOfInsertedInstitution)
                    < 0)
            {
                codesArray[j + 1] = codesArray[j];
                codesArray[j] = elementToInsert;
                
                numberOfMovimentations++;
                institutionsList.set(institutionsList.get(j), j + 1);
                institutionsList.set(objectToInsert, j);
            }
            
            else
            {
                codesArray[j + 1] = elementToInsert;
                institutionsList.set(objectToInsert, j + 1);
            }
            
            numberOfMovimentations++; // conta a ultima movimentacao
        }
    ...
    /*public static int binarySearch(int key, int esq, int dir, int[] array)
    {
        ...
        while (esq <= dir)
        {
            ...
            if (key == mid)
            {
                insertionIndex = midIndex + 1;
                esq = dir + 1; // termina o while
            }
    ...
    public static void insertion(int[] array)
    {
        ...
        int insertingElement;
        int insertingIndex;
        int j;
        
        for (int i = 1; i < length; i++)
        {
            insertingElement = array[i];
            insertingIndex = binarySearch(insertingElement, 0, i, array);
            
            for (j = i - 1; j >= insertingIndex; j--)
            {
                array[j + 1] = array[j];
            }
            
            array[j + 1] = insertingElement;
        }
    }*/
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        ordenationResult = ordenateInstitutionsByCode(listaDeInstituicoes, 10).split("\t");
        
        numberOfComparisons = AxellIO.getint(ordenationResult[0]);
        numberOfMovimentations = AxellIO.getint(ordenationResult[1]);
        
        listaDeInstituicoes.printList(0, 10);
        
        AxellIO.setPrintStream("matrícula_insercao.txt");
