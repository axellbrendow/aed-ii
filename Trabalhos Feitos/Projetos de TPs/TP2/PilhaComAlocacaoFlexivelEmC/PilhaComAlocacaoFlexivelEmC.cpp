...
Stack *createStack()
{
    return (Stack *) createDoubleList();
}

void stackUp(Stack *stack, void *element)
{
    addOnEnd((DoubleList *) stack, element);
}

void *unstack(Stack *stack)
{
    return removeOnEnd((DoubleList *) stack);
}

void freeStack(Stack *stack)
{
    freeDoubleList((DoubleList *) stack);
}
...
Instituicao *clone(Instituicao *instituicao)
{
    ...
    *clon = *instituicao; // risco de copia superficial (shallow copy)
...
PUC Minas - Ciencia da Computacao     Nome: PilhaComAlocacaoFlexivelEmC
...
void printInstitutions(Stack *stack)
{
    int stackSize = size((DoubleList *) stack);
    int i;
    
    for (i = 0; i < stackSize; i++)
    {
        imprimir( (Instituicao *) getCell((DoubleList *) stack, i)->element );
        puts("");
    }
}
...
int main()
{
    ...
    while (lineNumber != 0)
    {
        ...
        stackUp( stack, criarInstituicao( fileLine ) );
    ...
    for (int i = 0; i < numberOfCommands; i++)
    {
        ...
        if (commandOperands->address[0] == 'I')
        {
            ...
            stackUp(stack, instituicaoAtual);
        }

        else if (commandOperands->address[0] == 'R')
        {
            printf( "(R) %s\n", getNome( (Instituicao *) unstack(stack) ) );
        }
