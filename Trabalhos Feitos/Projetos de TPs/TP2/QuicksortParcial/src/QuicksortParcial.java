...
PUC Minas - Ciencia da Computacao     Nome: QuicksortParcial
...
{
    public static String[] getInstitutionsNames(Lista institutionsList)
    {
        ...
        {
            institutionsNames[i] = Instituicao.convertToInstituicao( institutionsList.get(i) ).getNome();
        }
    ...
    public static String partialQuicksort(int esqInicial, int dirInicial, String[] stringArray, Lista institutionsList, String lastResult, int k)
    {
        ...
        int numberOfComparisons = AxellIO.getint(lastResultSplited[0]);
        int numberOfMovimentations = AxellIO.getint(lastResultSplited[1]);
        String result;
        ...
        int esqLength = 0; // guardara' o tamanho da particao da esquerda
        
        if (direita > esqInicial)
        {
            esqLength = direita - esqInicial + 1; // atualiza o tamanho da particao da esquerda
            result = partialQuicksort(esqInicial, direita, stringArray, institutionsList, result, k);
        }
        
        // checa se o tamanho da particao da esquerda e' menor que k
        if (esqLength < k && esquerda < dirInicial)
        {
            result = partialQuicksort(esquerda, dirInicial, stringArray, institutionsList, result, k);
        }
    ...
    public static String partialQuicksort(String[] stringArray, Lista institutionsList, int k)
    {
        String result = "0\t0";
        
        if (stringArray == null)
        {
            //AxellIO.println("[QuicksortParcial]: Arranjo nulo. - quicksort(String[])");
        }
        
        else
        {
            result = partialQuicksort(0, stringArray.length - 1, stringArray, institutionsList, result, k);
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        ordenationResult = partialQuicksort(getInstitutionsNames(listaDeInstituicoes), listaDeInstituicoes, k).split("\t");
        
        numberOfComparisons = AxellIO.getint(ordenationResult[0]);
        numberOfMovimentations = AxellIO.getint(ordenationResult[1]);
        
        listaDeInstituicoes.printList(0, k);
        
        AxellIO.setPrintStream("matrícula_quicksort.txt");
