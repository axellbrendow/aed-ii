...
class List
{
    ...
    public Cell getCellOnIndex(int index)
    {
        ...
        {
            AxellIO.println("[SimpleList]: Indice invalido. (index = " + index + ") - funcao getCellOnIndex(int)");
        }
    ...
    public Object remove(int index)
    {
        ...
        {
            removedElement = removeOnEnd();
        }
        
        else if (index == 0)
        {
            removedElement = removeOnStart();
        }
        
        else
        {
            Cell previousOfRemovedCell = getCellOnIndex(index - 1);
            
            if (previousOfRemovedCell != null)
            {
                Cell removedCell = previousOfRemovedCell.getNext();
                Cell nextOfRemovedCell = removedCell.getNext();
                removedElement = removedCell.getElement();
                
                previousOfRemovedCell.setNext(nextOfRemovedCell);
                nextOfRemovedCell.setPrevious(previousOfRemovedCell);
            
                removedCell.setNext(null);
                removedCell.setPrevious(null);
                removedCell = null;
            }
            
            previousOfRemovedCell = null;
        }
    ...
    {
        Cell lastCell = getLast();
        Cell newCell = new Cell(element, lastCell, null);
    ...
    public void add(Object element, int index)
    {
        ...
        {
            AxellIO.println("[SimpleList]: Indice invalido. (index = " + index + ") - funcao add(Object, int)");
        }
        ...
        {
            Cell previousOfCellOnIndex = index == 0 ? getHead() : getCellOnIndex(index - 1);
            Cell cellOnIndex = previousOfCellOnIndex.getNext();
            Cell newCell = new Cell(element, previousOfCellOnIndex, cellOnIndex);
            
            previousOfCellOnIndex.setNext(newCell);
            cellOnIndex.setPrevious(newCell);
            newCell.setNext(cellOnIndex);
            newCell.setPrevious(previousOfCellOnIndex);
            
            newCell = null;
            cellOnIndex = null;
            previousOfCellOnIndex = null;
        }
    }
    ...
    {
        Cell copyOf1 = getCellOnIndex(index1);
        Cell copyOf2 = getCellOnIndex(index2);
        
        remove(index1);
        add(copyOf2.getElement(), index1);
        
        remove(index2);
        add(copyOf1.getElement(), index2);
    }
    ...
    public void printList()
    {
        if (!isEmpty())
        {
            ...
            AxellIO.println(currentCell.getElement().toString());
...
PUC Minas - Ciencia da Computacao     Nome: QuicksortListaDupla
...
{
    public static String[] getInstitutionsNames(List institutionsList)
    {
        int size = institutionsList.size();
        String[] institutionsNames = new String[size];
        
        for (int i = 0; i < size; i++)
        {
            institutionsNames[i] = Instituicao.convertToInstituicao( institutionsList.getCellOnIndex(i).getElement() ).getNome();
        }
    ...
    public static String quicksort(int esqInicial, int dirInicial, String[] stringArray, List institutionsList, String lastResult)
    {
        ...
        String[] lastResultSplited = lastResult.split("\t");
        String result;
        ...
        while (esquerda <= direita)
        {
            ...
            */
            if (esquerda <= direita)
            {
                ...
                institutionsList.swap(esquerda++, direita--);
            }
        }
        
        numberOfComparisons += AxellIO.getint(lastResultSplited[0]);
        numberOfMovimentations += AxellIO.getint(lastResultSplited[1]);
        
        result = numberOfComparisons + "\t" + numberOfMovimentations;
        
        if (direita > esqInicial)
        {
            result = quicksort(esqInicial, direita, stringArray, institutionsList, result);
        }
        
        if (esquerda < dirInicial)
        {
            result = quicksort(esquerda, dirInicial, stringArray, institutionsList, result);
        }
    ...
    public static String quicksort(String[] stringArray, List institutionsList)
    {
        String result = "0\t0";
        
        if (stringArray == null)
        {
            //AxellIO.println("[Quicksort]: Arranjo nulo. - quicksort(String[])");
        }
        
        else
        {
            result = quicksort(0, stringArray.length - 1, stringArray, institutionsList, result);
        }
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        List listaDeInstituicoes = new List();
        ...
        ordenationResult = quicksort(getInstitutionsNames(listaDeInstituicoes), listaDeInstituicoes).split("\t");
        
        numberOfComparisons = AxellIO.getint(ordenationResult[0]);
        numberOfMovimentations = AxellIO.getint(ordenationResult[1]);
        
        listaDeInstituicoes.printList();
        
        AxellIO.setPrintStream("matrícula_quicksort2.txt");
